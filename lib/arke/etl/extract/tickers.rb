  ...
  class Tickers < Base
    ...
    def start
      EM::Synchrony.add_periodic_timer(5) do
        # TODO: Don't publish old tickers
        @influxdb.query("SELECT MAX(price) AS high, MIN(price) AS low, LAST(price) as last, FIRST(price) AS open, MEAN(price) AS avg_price, SUM(total) AS volume, (LAST(price) - FIRST(price)) / FIRST(price) * 100 AS price_change_percent INTO tickers FROM trades WHERE time > now() - 24h GROUP BY market")
