...
module Arke::Strategy
  class Skew < Base
    ...
    attr_reader :limit_asks_base, :limit_bids_quote
    ...
    def initialize(sources, target, config, reactor)
      ...
      @target_base_balance_in_quote = params["target_base_balance_in_quote"]&.to_d
      @max_skew_perc = params["max_skew_perc"]&.to_d || "0.5".to_d

      logger.info "ID:#{id} levels_price_step: %.2f" % [@levels_price_step]
      logger.info "ID:#{id} levels_count: %.0f" % [@levels_count]
      logger.info "ID:#{id} spread_bids: %.3f%%" % [@spread_bids * 100]
      logger.info "ID:#{id} spread_asks: %.3f%%" % [@spread_asks * 100]
    ...
    def check_config
      ...
      raise "target_base_balance_in_quote missing" if @target_base_balance_in_quote.nil?
      raise "max_skew_perc must lower than one" if @max_skew_perc > 1
      raise "max_skew_perc must be higher than zero" if @max_skew_perc <= 0
    end
    ...
    def get_skew(mid_price)
      balance_quote = target.account.balance(target.base)["total"] * mid_price
      skew = (@target_base_balance_in_quote - balance_quote) / @target_base_balance_in_quote
      return @max_skew_perc if skew > @max_skew_perc
      return -@max_skew_perc if skew < -@max_skew_perc
    ...
    def call
      ...
      logger.info("ID:#{id} shape: %s skew: %f" % [shape, skew])

      best_ask_price = apply_spread(:sell, mp, @spread_asks) + skew * @levels_price_step
      best_bid_price = apply_spread(:buy, mp, @spread_bids) + skew * @levels_price_step
