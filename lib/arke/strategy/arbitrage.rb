  ...
  class Arbitrage < Base
    ...
    def trigger_orders(s1, s2, top_ask, top_bid, amount)
      ...
      if b2.nil? || b2["free"] < bid.amount * bid.price
        logger.warn("ID:#{id} Not enough %s to execute %s %s: %s" % [s2.quote, s2.account.id, s2.id, bid.to_s])
        return
      end

      b1 = s1.account.balance(s1.base)
      if b1.nil? || b1["free"] < ask.amount
        logger.warn("ID:#{id} Not enough %s to execute %s %s: %s" % [s1.quote, s1.account.id, s1.id, ask.to_s])
        return
      end
