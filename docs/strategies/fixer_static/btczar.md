...
accounts:
#---------------------------{source}-----------------------------------
- id: kraken_source                   # Unique account id of the source
  driver: kraken                      # One of supported sources drivers
...
strategies:
#---------------------------{strategy}-----------------------------------
- id: copy-BTCZAR                     # Name of a strategy
...
#---------------------------{fixer-dynamic}-----------------------------------
  fx:
    type: static                      # Set static for fixed conversion rate
    rate: 14.86                       # Convertion rate
  target:                           
    account_id: rubykube_target       # Unique account id, from the account section, that will be used as a target (your exchange)
    market_id: btczar                 # Market pair code in lowercase, from your target exchange
  sources:
  - account_id: kraken_source         # Unique account id, from the account section, that will be used as a source
