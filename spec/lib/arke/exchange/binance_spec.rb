describe Arke::Exchange::Binance do
  ...
  context 'ojbect initialization' do
    ...
    it 'has an orderbook' do
      expect(binance.orderbook).to be_an_instance_of(Arke::Orderbook::Orderbook)
    end
  end
  ...
  context "fetch_openorders" do
    it "fetchs the account openorders and stores them into local openorder cache" do
      binance.fetch_openorders
      expect(binance.open_orders[:buy].size).to eq(1)
      expect(binance.open_orders[:sell].size).to eq(0)
      expect(binance.open_orders[:buy][0.1]).to eq({42 => Arke::Order.new("LTCBTC", 0.1, 0.9, :buy)})
    end
  end
  ...
  context "get_amount" do
    ...
    it "get min_notional" do
      expect(binance.get_min_notional).to eq 10
    end
    ...
    it 'where order notional more than 20% from min_notional' do
      expect(binance.get_amount(danger_order)).to eq 3.3334
    end
